...
pdf("~/Pictures/lakes-biplot.pdf", 5, 5)
biplot(
  pca,
  scale = FALSE,
...
pdf("~/Pictures/lakes-pca.pdf", 8, 4)
...
# connectance ----------------
pdf("~/Pictures/lakes-metrics.pdf", 8, 4)
par(mfrow = c(1, 2), mar = c(4, 4, 1, 1))
...
scatter.smooth(
  ...
  frame = FALSE,
  xlab = "Species similarity (Jaccard)",
  ylab = "Difference in % basal species",
  lpars = list(col = "grey20", lwd = 5, lty = 1)
)
dev.off()
...
pdf("~/Pictures/lakes-dissimilarities.pdf", 4, 4)
par(mar = c(4, 4, 1, 1))
scatter.smooth(
  jaccard[lower.tri(jaccard)],
...
```{r bootstrap}
pdf("~/Pictures/lakes-bootstrap.pdf", 4, 4)
par(mar = c(4, 4, 1, 1))
plot(
  c(0, 0.55), c(0, 11),
  cex = 0,
...
summary(slopes)
pdf("~/Pictures/lakes-slopes.pdf", 4, 4)
par(mar = c(4, 4, 1, 1))
hist(
  slopes,
  breaks = seq(
